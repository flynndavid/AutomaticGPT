...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // Map to track grace-period timers for each operation
  const operationTimers = useRef<Map<string, ReturnType<typeof setTimeout>>>(new Map());
  ...
  // Helper to clear a timer for a specific operation
  const clearOperationTimer = (operationId: string) => {
    const timer = operationTimers.current.get(operationId);
    if (timer) {
      clearTimeout(timer);
  ...
  // Optimistic completion: immediately clear loading state after grace period
  const optimisticComplete = (operationId: string, nextState: Partial<AuthState>) => {
  ...
  const loadProfile = useCallback(
    ...
    [] // Now stable since it doesn't depend on changing state
  );
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      try {
        ...
        // First, try to get the session from the local storage (fast)
        const { session, error } = await auth.getSession();
        const fetchTime = Date.now() - startTime;
        console.log(`[AUTH] Session fetch completed in ${fetchTime}ms`);
        ...
        // Set auth state immediately with the cached session
        if (mounted) {
          console.log('[AUTH] Setting initial auth state:', {
        ...
        // If we have a session, validate it in the background
        if (session && mounted) {
          // Validate the session asynchronously without blocking
          auth
            .getUser()
            .then(({ user, error: userError }) => {
              if (userError || !user) {
            ...
            .catch((err) => {
              console.warn('[AUTH] Error validating session:', err);
            });
        }
          ...
          try {
            const {
              ...
              if (mountedRef.current) {
                // Clear any pending grace-timers because we received the event
  ...
  const signOut = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      // Start a 5-second grace timer – if the auth listener hasn't responded
      // by then we complete optimistically so the UI never blocks.
      const timer = setTimeout(() => {
        console.warn('[AUTH] Grace timer triggered for signOut – completing optimistically');
        optimisticComplete(operationId, {
          isAuthenticated: false,
          user: null,
          session: null,
          profile: null,
      ...
      operationTimers.current.set(operationId, timer);

      const { error } = await auth.signOut();
      if (error) throw error;
      // Do not complete here – the auth listener will resolve normally and
      // clear the timer. If it doesn't fire the timer handles it.
    } catch (error) {
      clearOperationTimer(operationId);
      completeOperation(operationId);
      setState((prev) => ({
  ...
  // Remove the old 30-second stuck-operation watchdog – the optimistic timers
  // make it unnecessary.  We keep a lightweight 10-second failsafe instead.
  useEffect(() => {
    if (activeOperations.size > 0) {
      const timeout = setTimeout(() => {
        console.warn(
          '[AUTH] Failsafe timeout – clearing active operations:',
