...
export function WeatherCard(props: { location: string; temperature: number }) {
  return (
    <View className="bg-white/90 p-3 rounded-lg w-full flex-row items-center gap-3">
      ...
      <View>
        <Text className="font-bold text-base text-zinc-800">Weather for {props.location}</Text>
        <Text className="text-zinc-600 text-base">{props.temperature}°F</Text>
      </View>
    </View>
  );
}
...
export function CelsiusConvertCard(props: { temperature: number; celsius: number }) {
  return (
    <View className="bg-white/90 p-3 rounded-lg w-full flex-row items-center gap-3">
      ...
      <View>
        <Text className="font-bold text-base text-zinc-800">Temperature Conversion</Text>
